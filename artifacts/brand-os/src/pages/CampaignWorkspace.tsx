  ...
  return new Promise((resolve) => {
    ...
    baseImg.onload = () => {
      ...
      if (logoUrl) {
        ...
        logoImg.onload = () => {
          // Logo in top-right corner: max 180x80px, with 20px margin
          const maxW = 160;
          const maxH = 80;
        ...
        // No logo — draw a colored badge with brand initials
    ...
    <div className="rounded-xl border border-card-border bg-card overflow-hidden flex flex-col">
      ...
      {displayImage ? (
        <div className="relative group">
          ...
          {/* Overlay badge showing logo is embedded */}
          {compositeImageUrl && (
            ...
            <div className="flex items-center gap-2">
              {!compositeImageUrl ? (
                ...
                >
                  ...
                  {compositing ? "Compositing..." : "Embed Logo"}
                </button>
              ) : (
                <button
        ...
        <div className="w-full aspect-video bg-gradient-to-br from-muted/60 to-muted/30 flex flex-col items-center justify-center gap-3 border-b border-card-border">
          ...
          <p className="text-[11px] text-muted-foreground">AI generates image + embed logo overlay</p>
        </div>
      )}
      ...
      {/* Panel tabs (only when variant or content exists) */}
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
              ...
              <p className="text-xs text-amber-700 dark:text-amber-300">
                This is your <strong>B variant</strong> — a different creative angle for A/B testing. Apply it to replace the current post content.
              </p>
            </div>
        ...
        {activePanel === "content" && longFormContent && (
          <div className="space-y-4">
            ...
            {longFormContent.metaDescription && (
              <div className="p-3 rounded-lg bg-muted/40">
                <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider mb-1">Meta Description (SEO)</p>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => copyText(longFormContent.content, "content")}
...
export default function CampaignWorkspace() {
  ...
  // Try to get brand info from campaign
  const brandLogoUrl = (campaign as unknown as { brand?: { logoUrl?: string } })?.brand?.logoUrl ?? undefined;
  const brandName = (campaign as unknown as { brand?: { companyName?: string } })?.brand?.companyName ?? "Brand";
  const brandPrimaryColor = "#6366F1"; // fallback
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-muted/40 border border-border">
              <Layers className="w-3 h-3" /> Logo overlay available
            </div>
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-muted/40 border border-border">
              <TestTube2 className="w-3 h-3" /> A/B testing
            </div>
          </div>
        </div>
